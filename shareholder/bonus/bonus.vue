<template>
  <view v-if="pageData" class="page-body">
    <div class="page-top bg-fff bdrd-20 p-t-40 p-x-24">
      ...
      <div class="flex-y-center flex-x-sa p-b-60 border-b">
        <div class="font-center">
          <div class="color-999 font-26">已提现佣金</div>
          <div class="font-600-38" style="color: #299b76">
            {{ pageData.cash_price }}
          </div>
        </div>
        <div class="font-center">
          <div class="color-999 font-26">累计佣金</div>
          <div class="font-600-38" style="color: #ff1b22">
            {{ pageData.total_price }}
          </div>
        </div>
      </div>

      <div class="flex-y-center flex-x-sa font-24 p-b-28 p-t-30">
        <div
          @click="$utils.toUrl('/other/cash/cash?type=shareholder')"
          class="flex-col flex-x-center flex-y-center"
        >
          <div class="iconfont font-50 theme-color icon-wallet"></div>
          <div>佣金提现</div>
        </div>
        <div
          @click="$utils.toUrl('/shareholder/a-bonus/a-bonus')"
          class="flex-col flex-x-center flex-y-center"
        >
          <div class="iconfont font-50 theme-color icon-daikuan"></div>
          <div>分红记录</div>
        </div>
        <div
          ...
          class="flex-col flex-x-center flex-y-center"
        >
          <div class="iconfont font-50 theme-color icon-jiehui"></div>
          <div>贡献值记录</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async loadData() {
      const res = await this.$requestAll.shareholder.bonusList({}, true);
