  ...
  <view v-if="pageData" class="page-body">
    <div class="page-top bg-fff bdrd-20 p-t-40 p-x-24">
      ...
      <div class="flex-y-center flex-x-sa p-b-60">
        <div class="font-center">
          <div class="color-999 font-26">自己消费贡献值</div>
          <div class="font-600-38" style="color: #299b76">
            {{ pageData.my_price }}
          </div>
        </div>
        <div class="font-center">
          <div class="color-999 font-26">推荐消费贡献值</div>
          <div class="font-600-38" style="color: #ff1b22">
            {{ pageData.recommend_price }}
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="detail p-24 bg-fff bdrd-20 m-t-24 flex-col">
      <div class="title flex-y-center flex-x-sb">
        <div class="left font-600-30">贡献值明细</div>
        <div class="right">
      ...
      <view>
        <div v-if="pageData.bonus_log.length">
          ...
          >
            <div class="left">
              <text v-if="item.status == 1" style="color: #ff1b22">获得</text>
              <text v-else style="color: #3678f2">扣除</text>
            </div>
            ...
            <div class="right font-right">
              <div>{{item.status == 1 ? '+': '-'}}{{item.amount}}</div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async loadData() {
      const res = await this.$requestAll.shareholder.bonusContributeList(
        {
          status: this.status,
        },
        true
