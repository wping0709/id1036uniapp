  ...
  <view v-if="pageData" class="body">
    ...
    <template v-if="pageData.is_agree == 1">
      <div class="express-block m-t-20" v-if="pageData.is_user_send == 0">
        ...
        <div v-if="pageData.is_dada" class="p-b-10">
          <div style="color: #999">快递信息选填</div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async loadData() {
      const res = await this.$requestAll.order.refundDetail(
    ...
    async sendFormSubmit() {
      console.log('tijiao');
      if (this.express_index == -1 && !this.pageData.is_dada)
        return uni.$u.toast('请选择快递公司');
      if (!this.express_no && !this.pageData.is_dada)
        return uni.$u.toast('快递单号不能为空');

      const formData = {
        order_refund_id: this.order_refund_id,
        express:
          (this.express_index != -1 &&
            this.pageData.express_list[this.express_index].name) ||
          '',
