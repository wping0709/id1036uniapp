  ...
  <view class="body">
    <div v-if="pageData.length" class="list">
      <div
        ...
        :key="item.id"
        class="item flex-y-center flex-x-sb p-20"
      >
        <view>
        ...
        <view
          @click="item.status == '1' && submit(item)"
          :class="['bt', item.status == 1 && 'bt-4']"
          v-else
        >
...
export default {
  ...
  methods: {
    async loadData() {
      ...
      const res = await this.$requestAll.scratch.scratchPrize(
        {
          page: this.page,
        },
        true
      ...
      this.page++;
      this.status == res.data.length < 10 ? 'nomore' : 'loadmore';
    },
    submit(goods) {
