<template>
  <view class="body flex-col">
    <view class="bg-fff">
      <u-tabs
        :current="status"
        :list="list1"
        :activeStyle="{
          color: 'red',
        }"
        lineColor="red"
        @change="clickTab"
      ></u-tabs>
    </view>

    <scroll-view
      @scrolltolower="scrolltolower"
      scroll-y
      class="content flex-h-1"
    >
      <view class="scroll-box" v-if="list.length">
        <view class="tixian-item" v-for="(item, index) in list" :key="index">
          <view class="flex-y-center flex-x-sb">
            <text>购买用户-{{ item.buy_user.nickname }}</text>
            <text class="price-color">￥{{ item.pay_price }}</text>
          </view>

          <view class="flex-y-center flex-x-sb font-26 color-999">
            <text>佣金</text>
            <text style="color: #ff7e00">￥{{ item.price }}</text>
          </view>

          <view class="flex-y-center flex-x-sb font-26 color-999">
            <text>{{ item.addtime }}</text>
            <text>{{ item.is_give > 0 ? '已结算' : '未结算' }}</text>
          </view>
        </view>
        <u-loadmore :status="more"></u-loadmore>
      </view>
      <view class="scroll-box" v-else>
        <u-empty
          mode="data"
          :icon="wxapp_img.empty_data"
        >
        </u-empty>
      </view>
    </scroll-view>
  </view>
</template>
<script>
export default {
  data() {
    return {
      list1: [
        {
          name: '全部',
        },
        {
          name: '未结算',
        },
        {
          name: '已结算',
        },
      ],
      pageData: '',
      status: 0,
      list: [],
      page: 1,
      more: 'loadmore',
      anchor_id: '',
    };
  },
  onLoad(params) {
    console.log(params);
    this.anchor_id = params.anchor_id;
    this.loadData();
  },
  methods: {
    async loadData() {
      if (this.more == 'nomore') return;
      const res = await this.$requestAll.lives.getShareList(
        {
          status: this.status - 1,
          page: this.page,
          anchor_id: this.anchor_id,
        },
        true
      );
      this.list.push(...res.data.list);
      this.more = res.data.list.length < 10 ? 'nomore' : 'loadmore';
      this.page++;
    },
    clickTab({ index }) {
      this.status = index;
      this.resetPageData();
      this.loadData();
    },
    scrolltolower() {
      this.page++;
      this.loadData();
    },
    resetPageData() {
      this.list = this.$options.data().list;
      this.page = this.$options.data().page;
      this.more = this.$options.data().more;
    },
  },
  onShow() {},
  computed: {},
  watch: {},
  onReachBottom() {},
};
</script>
<style lang="scss" scoped>
.body {
  background: #f5f5f5;
  height: 100vh;
  /* #ifdef H5 */
  height: calc(100vh - 44px);
  /* #endif */
}
.tixian-item {
  margin-top: 20rpx;
  padding: 20rpx;
  background: #fff;
  line-height: 1.7;
}
</style>
