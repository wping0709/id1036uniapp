    ...
    <div class="list">
      <div class="user" v-for="(item, index) in pageData.list">
        <div class="info">
          ...
          >
            <img
              :src="item.user_headimg ? item.user_headimg : ''"
              alt=""
            />
          </div>
          <div>
        ...
        <div class="praise">
          ...
          <div class="hint">评论了你：</div>
          <div class="cont">{{ item.review }}</div>
        </div>
        <div
          class="dynamic"
          @click="
            $utils.toUrl('/weitao/details/details?article_id=' + item.article_id)
          "
        >
          {{ item.return_content }}
        </div>
...
export default {
  ...
  methods: {
    async loadData() {
      const res = await this.$requestAll.weitao.getNotReview({}, true);
