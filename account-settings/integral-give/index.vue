<template>
  <view v-if="userInfo" class="page-body">
    <div class="bg-fff bdrd-20 p-y-30 p-x-24 m-b-24">
      <view class="font-600-30 m-b-20">转增数量</view>
      ...
      <view class="m-t-30 font-24 color-999">
        <text>可转赠{{ userInfo.integral }}积分，</text>
        <text @click="form.number = userInfo.integral" style="color: #ff1b22"
          >全部转赠</text
        >
      </view>
    </div>
    ...
    <div class="p-x-24 color-999 font-24 m-b-24">
      积分转增手续费{{ store.option.integral_converting_charge || 0 }}%
    </div>
...
export default {
  ...
  computed: {
    ...
    submitDisplay() {
      return (
        !this.giveInfo ||
        Number(this.form.number) > Number(this.userInfo.integral) ||
        this.form.number <= 0
      );
    },
  },
