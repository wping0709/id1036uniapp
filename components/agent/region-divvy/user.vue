<template>
    <view v-if="storeData.custom" class="body">
        <template>
             <view class="head"   :style="storeData.background_image ? 'background-image: url(' + storeData.background_image + '); background-size: cover; background-position: center center; background-repeat: no-repeat;' : 'background-color: red;'">
                <view class="user-info flex-y-center">
                    ...
                    <div class="flex-w-1 flex">
                        ...
                        <div @click="showXuzhi = true" class="iconfont icon-ziyuan1 m-l-20 font-40"></div>
                    </div>
                </view>
                ...
                <view class="price flex-x-sb flex-y-center">
                    ...
                    <view class="price-bt" @click="$utils.toUrl('/other/cash/cash?type=region')">
                        {{
            ...
            <view class="menu-1">
                ...
                <view class="font-center">
                    ...
                    <view class="m-t-10">{{ storeData.price.un_pay }}元</view>
                </view>
            </view>
            ...
            <view class="menu-2" v-if="storeData.custom && storeData.custom.menus">
                <template v-for="(item, i) in storeData.custom.menus">
                    ...
                    <!-- 分销订单 -->
                    <view v-if="i == 'order'" :key="i" @click="$utils.toUrl('/agent/region-divvy/order')"
                    ...
                    <!-- 提现明细 -->
                    <view v-if="i == 'cash'" :key="i" @click="
                        $utils.toUrl(
                            '/pages/cash-detail/cash-detail?type=region&title=' + item.name
                        )
                        " class="flex-col flex-y-center flex-x-center font-center">
                        <image :src="item.icon" mode="aspectFit" />
                        <view class="m-y-10">{{ item.name }}</view>
        ...
        <u-popup round="20rpx" closeable :show="showXuzhi" :safe-area-inset-bottom="false" @close="showXuzhi = false"
            mode="center">
...
export default {
    ...
    methods: {
        async loadData() {
            const res1 = await this.$requestAll.agentApproval.getInfo({}, true);
            // 用来获取分销设置
            uni.setStorageSync('user_type', res1.data);
            uni.getStorageSync('user_type');
            ...
            console.log('this.storeDatathis.storeDatathis.storeData', this.storeData);
        },
    },
    onShow() { },
    computed: {
