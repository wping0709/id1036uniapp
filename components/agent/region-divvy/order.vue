    ...
    <view class="body flex-col">
        <view class="bg-fff">
            <u-tabs :current="status" :list="list1" :activeStyle="{
                color: 'red',
            }" lineColor="red" :scrollable="false" @change="clickTab"></u-tabs>
        </view>

        <scroll-view @scrolltolower="loadData" scroll-y class="content flex-h-1">
            <view class="scroll-box" v-if="list.length">
                <div class="list">
                    <div class="item bg-fff m-b-20 m-x-20 p-x-20" v-for="(item, i) in list" :key="i">
                        ...
                        <view @click="onClickItem(item)" class="flex-y-center flex-x-sb">
                            ...
                            <view class="flex-y-center">
                                <text>
                                    预计佣金
                                    <text class="price-color">{{ item.commission }}</text>
                                    元
                                </text>
                        ...
                        <template>
                            <view class="goods flex-y-center" v-for="(goods, i1) in item.orderDetail" :key="i1">
...
export default {
    ...
    methods: {
        async loadData() {
            ...
            const res = await this.$requestAll.agentApproval.getOrder(
                {
