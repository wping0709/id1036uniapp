<template>
  <view class="body flex-col">
    <view class="flex-y-center flex-x-sb m-x-20">
      <text>线下购物车</text>
      <text @click="onSaoyisao" class="iconfont icon-saoyisao font-60"></text>
    </view>
    ...
    <scroll-view scroll-y class="flex-h-1">
      ...
      <view
        ...
        v-if="userInfo && userInfo.level == -1 && memberPrice.unitPrice > 0"
        class="huiyuan"
      >
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      const res = await this.$requestAll.cart.cartOfflineList({}, true);
    ...
    async numChange({ value }, index, key, mchIndex) {
      ...
      await this.$requestAll.cart.cartOfflineEdit(formData);
      // });
    },
    // 全选chenge
    clickQuanxuan() {
    ...
    submit() {
      ...
      if (this.isModify) {
        ...
        uni.showModal({
          ...
          success: async res => {
            if (res.confirm) {
              await this.$requestAll.cart.cartOfflineDelete({
      ...
      // 当前为结算
        ...
        this.$utils.toUrl(
          `/pages/new-order-submit-offline/new-order-submit-offline?mch_list=${JSON.stringify(
            mch_list
          )}&isNewOrder=true`
        );
      }
