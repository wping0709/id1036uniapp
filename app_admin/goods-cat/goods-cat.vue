<template>
  <view v-if="catList" class="body flex-col">
    <view class="p-y-10">
      <view
        @click="$utils.toUrl('/app_admin/add-cat/add-cat')"
    ...
    <view class="env-bottom">
      ...
      <view v-for="(cat, i) in catList" :key="i" class="bg-fff cat-box">
        ...
        <div class="cat-item flex-y-center">
          <image :src="cat.pic_url" mode="aspectFill" />
          ...
          <view class="more-box">
            ...
            >
              <view
                @click="$utils.toUrl('/app_admin/add-cat/add-cat?id=' + cat.id)"
              ...
              >
                <text
                  v-if="cat.is_show == 1"
                  class="iconfont icon-yincang font-50"
                ></text>
                <text
                  v-else
                  class="iconfont icon-yanjing_xianshi font-50"
                ></text>
            ...
            <div class="cat-item flex-y-center">
              ...
              <view class="more-box">
                ...
                >
                  <view
                    @click="
                      $utils.toUrl('/app_admin/add-cat/add-cat?id=' + cat.id)
                    "
                    class="flex-col flex-x-center flex-y-center p-x-20 p-y-10"
                  >
                    <text class="iconfont icon-bianji font-50"></text>
                  ...
                  <view class="more-box">
                    ...
                    >
                      <view
                        @click="
                          $utils.toUrl(
                            '/app_admin/add-cat/add-cat?id=' + cat.id
                          )
                        "
                        class="flex-col flex-x-center flex-y-center p-x-20 p-y-10"
                      >
...
export default {
  ...
  methods: {
    async loadData() {
      const res = await this.$requestAll.admin.catCat({}, true);
    ...
    delCatItem(cat) {
      uni.showModal({
        ...
        success: async res => {
          if (res.confirm) {
            await this.$requestAll.admin.catDelete({ id: cat.id }, true);

            // 递归剔除当前选中项
            var digui = function (list, id) {
              let result = [];
              ...
              for (let i = 0; i < list.length; i++) {
                ...
                if (item.id != id) {
                  if (item.child && item.child.length) {
                    item.child = digui(item.child, id);
                  }
                  result.push(item);
                }
              }
            ...
            this.catList = digui(this.catList, cat.id);
          }
        },
      });
    },
    // 隐藏、显示切换
    async catStatus(item) {
      var formData = {
        id: item.id,
        status: item.is_show == 1 ? 2 : 1,
      };
      await this.$requestAll.admin.catStatus(formData, true);
      uni.$u.toast('切换成功');
      // 递归修改当前选中项
      var digui = function (list, id, status) {
        let result = [];
        for (let i = 0; i < list.length; i++) {
          let item = list[i];
          if (item.id == id) {
            ...
            item.child = digui(item.child, id, status);
          }
          result.push(item);
        }
        return result;
      };
      this.catList = digui(this.catList, item.id, formData.status);
    },
    catSwitchFold(cat) {
      console.log(cat);
