<template>
  <view class="body flex-col">
    <div class="search-box flex-y-center">
      ...
      <div
        @click="$utils.toUrl('/app_admin/add-goods/add-goods')"
...
export default {
  ...
  methods: {
    async loadData(isReset) {
      ...
      const res = await this.$requestAll.admin.goodsList({
    ...
    async switchStatus(goods) {
      const res = await this.$requestAll.admin.goodsSwitch({
        id: goods.id,
    ...
    clickDelete(goods) {
      ...
      uni.showModal({
        ...
        success: async function (res) {
          if (res.confirm) {
            const res = await that.$requestAll.admin.goodsDel(
              {
                id: goods.id,
              },
              true
